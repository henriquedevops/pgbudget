...
> **Atualização 2026-06-13:** U5, U13, U14, U11, U12 e U15 resolvidos; U7 em grande parte (só i18n) e U10 parcial — status por item nas seções 3 e 4
...
**Status:** 🟡 **Em grande parte resolvido em 2026-06-13** — a saudação e o card de usuário agora exibem o **nome real** do usuário (`pgb_display_name()` em `config/database.php` busca `first_name` e cacheia na sessão; fallback para o username) em vez do identificador interno; o rótulo "Ledger" da sidebar virou "Budget" (o menu já usava "Bills" para obligations). Sweep do glossário "Budget vs Ledger" nas strings visíveis: `ledgers/create.php` ("Set up a new zero-sum budget"), `accounts/reconcile.php` ("Budget:"), `settings/action-history.php` ("All Budgets"), `index.php` ("create your first budget"), `settings/index.php` ("Manage and create budgets" + texto de ajuda esclarecendo que "ledger" só aparece em URLs/API) e mensagens da API `delete-ledger.php` ("Budget deleted/not found"). Pendente: i18n das strings (toda a UI segue em inglês); termos "ledger" remanescentes ficam só em URLs (`?ledger=`), código e payloads internos da API, por design.
...
**Status:** 🟡 **Parcialmente resolvido em 2026-06-13** — os padrões repetidos do hero/stats do dashboard viraram classes utilitárias no design kit (`components.css`): `.dashboard-hero-grid` (grid base, antes inline duplicando a regra mobile), `.stat-value` (4 ocorrências de `font-size:var(--text-xl);font-weight:700`) e `.btn-on-hero` (3 botões translúcidos do hero). Pendente: o restante dos `style=` inline (incluindo os blocos `<style>` por página — ligado ao U6) e os utilitários `display:none` controlados por JS (deixados de fora por serem funcionais, não de estilo).
...
**Status:** ✅ **Resolvido em 2026-06-13** — (1) as células editáveis do orçamento (`.budget-amount-editable`) agora são focáveis (`tabindex="0"`, `role="button"`, `aria-label` com o nome da categoria) e abrem o editor inline via **Enter/Espaço** (`public/js/budget-inline-edit.js`), com `:focus-visible` em `components.css`; (2) os ícones de categoria/avatares (emojis e iniciais em `.cat-icon`/`.category-icon`, em `budget/dashboard.php`, `transactions/list.php`, `categories/manage.php` e `includes/rail.php`) receberam `aria-hidden="true"` — são decorativos e sempre acompanhados do nome textual visível, então o leitor de tela anuncia só o nome, sem o emoji redundante; (3) o botão de busca icon-only ganhou `aria-label="Search"`. Os botões de undo/redo já têm texto visível via `<span class="label">`.
...
| 8 | U7 — Glossário de terminologia | Médio | Médio | 🟡 Grande parte 2026-06-13 (só i18n pendente) |
...
| 10d | U15 — Acessibilidade (células editáveis + ícones) | Baixo | Baixo-Médio | ✅ 2026-06-13 |
| 10e | U10 — Estilos inline pós design kit | Baixo | Baixo | 🟡 Parcial 2026-06-13 (hero/stats) |

**Quick wins da semana:** ~~itens 1–4~~ ✅ concluídos em 2026-06-12, junto com o item 5 (U2) e o item 6 (U3 — navegação única + ledger na sessão). Em 2026-06-13 foram concluídos: item 7 (U5 — sweep alert/confirm), U13 (busca de transações), U14 (auto-dismiss proporcional), U11 (ação destrutiva no overflow), U12 (toggle de dark mode) e U15 (acessibilidade); U7 ficou em **grande parte** resolvido (glossário "Budget" varrido na UI, só faltando i18n) e U10 **parcial** (hero/stats do dashboard viraram utilitários). **Resta:** o i18n das strings da UI (cobre o remanescente de U2 e U7), o item 9 (U6 — refactor do `add.php`) e o complemento do U10 (demais `style=` inline e blocos `<style>` por página).
