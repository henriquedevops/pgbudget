-- +goose Up
-- Split section 6 of generate_cash_flow_projection so that monthly recurring
-- transactions show in ALL months within the report range, not just from next_date
-- forward.  The key change:
--   6a  backfills months [p_start_month .. date_trunc(next_date) - 1 month] for
--       monthly-frequency transactions that have already started (next_date is
--       within the report range).
--   6b  keeps the original behaviour for ALL frequencies from next_date onward.
--
...
begin
    ...
    -- 6a. RECURRING TRANSACTIONS — monthly, backfill past months before next_date
    --     Shows months [p_start_month .. date_trunc(next_date) - 1 month] so that
    --     past months where the transaction already fired appear in the report.
    --     Guard: only applies when next_date is within the report window
    --     (prevents future-only transactions from polluting past months).
    select
    ...
    cross join lateral generate_series(
        ...
        least(
            date_trunc('month', rt.next_date::timestamp) - interval '1 month',
    ...
    where rt.ledger_id = v_ledger_id and rt.user_data = v_user_data and rt.enabled = true
      ...
      and date_trunc('month', rt.next_date::timestamp) > date_trunc('month', p_start_month::timestamp)
      and date_trunc('month', rt.next_date::timestamp) <= date_trunc('month', v_end_month::timestamp)

    union all

    -- 6b. RECURRING TRANSACTIONS — all frequencies, from next_date onwards
...
-- +goose Down
-- Restore section 6 to previous single-block behaviour (no past-month backfill).
-- +goose StatementBegin
create or replace function api.generate_cash_flow_projection(
    p_ledger_uuid text,
    p_start_month date default date_trunc('month', current_date)::date,
