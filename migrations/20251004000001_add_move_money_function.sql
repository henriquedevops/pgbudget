-- +goose Up
-- Add function to move money between categories (YNAB Rule 3: Roll With The Punches)

-- +goose StatementBegin
-- utils function: move_between_categories
...
-- +goose StatementBegin
-- api function: move_between_categories
-- public wrapper for utils function
...
) returns text as $apifunc$
declare
    v_transaction_uuid text;
begin
    -- call utils function with current user context
    v_transaction_uuid := utils.move_between_categories(
        p_ledger_uuid := p_ledger_uuid,
        p_from_category_uuid := p_from_category_uuid,
        p_to_category_uuid := p_to_category_uuid,
        p_amount := p_amount,
        p_date := p_date,
        p_description := p_description,
        p_user_data := utils.get_user()
    );
...
-- +goose Down
-- Remove move money functions
drop function if exists api.move_between_categories(text, text, text, bigint, timestamptz, text);
drop function if exists utils.move_between_categories(text, text, text, bigint, timestamptz, text, text);
