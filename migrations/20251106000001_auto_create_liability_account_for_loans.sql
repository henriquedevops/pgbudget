...
-- ============================================================================
-- UPDATE api.create_loan TO AUTO-CREATE LIABILITY ACCOUNT
-- ============================================================================
-- This migration updates the api.create_loan function to automatically create
-- a liability account for each loan, eliminating the need for users to
-- manually create and link accounts.
-- ============================================================================

-- Drop the existing function first
DROP FUNCTION IF EXISTS api.create_loan(text, text, text, numeric, numeric, integer, date, date, text, text, text, text, integer, text, text, numeric, date, integer);
...
CREATE OR REPLACE FUNCTION api.create_loan(
    ...
    p_account_uuid text DEFAULT NULL,  -- Now optional, will be auto-created if NULL
...
BEGIN
    ...
    IF p_account_uuid IS NOT NULL THEN
        ...
        END IF;
    ELSE
        -- Auto-create a liability account for this loan

        -- Map loan_type to liability_subtype
        v_liability_subtype := CASE p_loan_type
            WHEN 'mortgage' THEN 'mortgage'
            WHEN 'auto' THEN 'loan'
            WHEN 'personal' THEN 'loan'
            WHEN 'student' THEN 'loan'
            WHEN 'credit_line' THEN 'line_of_credit'
            WHEN 'other' THEN 'other'
            ELSE 'other'
        END;

        -- Generate account name
        -- Format: "{Lender Name} {Loan Type}" (e.g., "Chase Mortgage", "Toyota Auto Loan")
        v_account_name := p_lender_name || ' ' ||
            CASE p_loan_type
                WHEN 'mortgage' THEN 'Mortgage'
                WHEN 'auto' THEN 'Auto Loan'
                WHEN 'personal' THEN 'Personal Loan'
                WHEN 'student' THEN 'Student Loan'
                WHEN 'credit_line' THEN 'Line of Credit'
                WHEN 'other' THEN 'Loan'
                ELSE 'Loan'
            END;

        -- Build metadata
        v_account_metadata := jsonb_build_object('liability_subtype', v_liability_subtype);

        -- Insert the liability account
        INSERT INTO data.accounts (
            ...
            v_account_name,
            'Auto-created for loan: ' || p_lender_name,
            'liability',
            'liability_like',
            v_account_metadata,
            v_ledger_id,
...
'Create a new loan with automatic liability account creation. If p_account_uuid is not provided, a liability account will be automatically created and linked to the loan.';
