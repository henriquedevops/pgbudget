-- +goose Up
-- Include installment number in cash-flow-projection description for installments.
-- Changes "MP*ALIEXPRESSO" → "MP*ALIEXPRESSO 12/12" so the user can tell which
-- installment is projected and verify it matches their CC invoice.
--
-- Based on the full function definition from 20260219000001_add_cc_payment_projection.sql
-- (amount bigint, includes cc_payment sections 8a/8b/8c).
...
begin
    ...
    -- Description includes installment number (e.g. "MP*ALIEXPRESSO 12/12")
    select
        date_trunc('month', s.due_date)::date, 'installment'::text, ip.id, ip.uuid,
        'expense'::text, 'installment'::text,
        ip.description || ' ' || s.installment_number::text || '/' || ip.number_of_installments::text,
...
-- +goose Down
-- Revert: remove installment number from description
-- +goose StatementBegin
create or replace function api.generate_cash_flow_projection(
    p_ledger_uuid text,
    p_start_month date default date_trunc('month', current_date)::date,
