-- +goose Up
-- Add recurrence support to projected events.
-- Allows events to repeat monthly, annually, or semiannually.

-- Step 1: Add columns to data.projected_events
alter table data.projected_events
    add column frequency text not null default 'one_time',
    add column recurrence_end_date date;

alter table data.projected_events
    add constraint projected_events_valid_frequency check (
        frequency in ('one_time', 'monthly', 'annual', 'semiannual')
    );

-- Step 2: Drop dependent functions and view so we can recreate with new columns
...
drop function if exists api.update_projected_event(text, text, text, text, text, bigint, text, date, text, boolean, boolean, text, text);
drop function if exists api.create_projected_event(text, text, bigint, date, text, text, text, text, text, boolean, text);
drop view if exists api.projected_events;

-- Step 3: Recreate view with new columns
create view api.projected_events as
select
    e.uuid,
    e.name,
    e.description,
...
-- Step 4: Recreate create function with frequency params
...
begin
    ...
    insert into data.projected_events (
        ...
        is_confirmed, notes, frequency, recurrence_end_date
    ) values (
        v_ledger_id, p_name, p_description, p_event_type, p_direction,
        p_amount, p_currency, p_event_date, v_category_id,
        p_is_confirmed, p_notes, p_frequency, p_recurrence_end_date
...
-- Step 5: Recreate update function with frequency params
-- +goose StatementBegin
...
-- Step 6: Recreate simple read/delete functions
-- +goose StatementBegin
...
-- Step 7: Update projection engine — expand section 7 to handle all frequencies
-- +goose StatementBegin
...
begin
    ...
    -- 7c. PROJECTED EVENTS: annual recurring (fires in the same calendar month each year)
    select
        gs.month::date, 'event'::text, e.id, e.uuid,
        e.event_type, e.event_type, e.name,
        case when e.direction = 'inflow' then e.amount else -(e.amount) end
    ...
    -- 7d. PROJECTED EVENTS: semiannual recurring (fires in event month + 6-month offset)
    -- E.g., event_date in March → fires every March and September
    select
        gs.month::date, 'event'::text, e.id, e.uuid,
        e.event_type, e.event_type, e.name,
        case when e.direction = 'inflow' then e.amount else -(e.amount) end
...
-- Grants (pgbudget_user needs execute on new function signatures)
grant execute on function api.create_projected_event(text, text, bigint, date, text, text, text, text, text, boolean, text, text, date) to pgbudget_user;
grant execute on function api.update_projected_event(text, text, text, text, text, bigint, text, date, text, boolean, boolean, text, text, text, date) to pgbudget_user;

-- +goose Down
-- Reverse: drop new functions/view, restore old versions

drop function if exists api.get_projected_event(text);
drop function if exists api.get_projected_events(text);
drop function if exists api.delete_projected_event(text);
drop function if exists api.update_projected_event(text, text, text, text, text, bigint, text, date, text, boolean, boolean, text, text, text, date);
drop function if exists api.create_projected_event(text, text, bigint, date, text, text, text, text, text, boolean, text, text, date);
drop view if exists api.projected_events;

-- Restore view without new columns
create view api.projected_events as
select
    e.uuid, e.name, e.description, e.event_type, e.direction, e.amount, e.currency, e.event_date,
    c.uuid as default_category_uuid, c.name as default_category_name,
    e.is_confirmed, e.is_realized, t.uuid as linked_transaction_uuid,
    e.notes, e.metadata, e.created_at, e.updated_at, l.uuid as ledger_uuid
from data.projected_events e
left join data.ledgers l on l.id = e.ledger_id
left join data.accounts c on c.id = e.default_category_id
left join data.transactions t on t.id = e.linked_transaction_id
...
-- Drop new columns
alter table data.projected_events
    drop constraint if exists projected_events_valid_frequency,
    drop column if exists frequency,
    drop column if exists recurrence_end_date;

-- Restore original functions (from 20260216000006)
-- +goose StatementBegin
create or replace function api.create_projected_event(
    p_ledger_uuid text, p_name text, p_amount bigint, p_event_date date,
    p_direction text default 'outflow', p_event_type text default 'other',
    p_description text default null, p_currency text default 'BRL',
    p_default_category_uuid text default null, p_is_confirmed boolean default false,
    p_notes text default null
) returns setof api.projected_events as $$
declare v_ledger_id bigint; v_category_id bigint; v_event_uuid text; v_user_data text;
begin
    v_user_data := utils.get_user();
    ...
    if p_default_category_uuid is not null then
        select id into v_category_id from data.accounts where uuid = p_default_category_uuid and user_data = v_user_data;
    end if;
    insert into data.projected_events (ledger_id, name, description, event_type, direction, amount, currency, event_date, default_category_id, is_confirmed, notes)
    values (v_ledger_id, p_name, p_description, p_event_type, p_direction, p_amount, p_currency, p_event_date, v_category_id, p_is_confirmed, p_notes)
    returning uuid into v_event_uuid;
    return query select * from api.projected_events where uuid = v_event_uuid;
end;
$$ language plpgsql security definer;
-- +goose StatementEnd
...
-- +goose StatementBegin
create or replace function api.update_projected_event(
    p_event_uuid text, p_name text default null, p_description text default null,
    p_event_type text default null, p_direction text default null, p_amount bigint default null,
    p_currency text default null, p_event_date date default null,
    p_default_category_uuid text default null, p_is_confirmed boolean default null,
    p_is_realized boolean default null, p_linked_transaction_uuid text default null,
    p_notes text default null
) returns setof api.projected_events as $$
declare v_event_id bigint; v_category_id bigint; v_transaction_id bigint; v_user_data text;
begin
    v_user_data := utils.get_user();
    ...
    if p_linked_transaction_uuid is not null then
        select id into v_transaction_id from data.transactions where uuid = p_linked_transaction_uuid and user_data = v_user_data;
    end if;
    update data.projected_events set
        name = coalesce(p_name, name), description = coalesce(p_description, description),
        event_type = coalesce(p_event_type, event_type), direction = coalesce(p_direction, direction),
        amount = coalesce(p_amount, amount), currency = coalesce(p_currency, currency),
        event_date = coalesce(p_event_date, event_date), default_category_id = coalesce(v_category_id, default_category_id),
        is_confirmed = coalesce(p_is_confirmed, is_confirmed), is_realized = coalesce(p_is_realized, is_realized),
        linked_transaction_id = coalesce(v_transaction_id, linked_transaction_id),
...
begin return query select * from api.projected_events where ledger_uuid = p_ledger_uuid order by event_date, name; end;
$$ language plpgsql security definer;
-- +goose StatementEnd

-- +goose StatementBegin
create or replace function api.get_projected_event(p_event_uuid text) returns setof api.projected_events as $$
begin return query select * from api.projected_events where uuid = p_event_uuid; end;
$$ language plpgsql security definer;
-- +goose StatementEnd

-- Restore projection engine to state from 20260218000001 (installment fix, no recurrence)
-- +goose StatementBegin
create or replace function api.generate_cash_flow_projection(
    p_ledger_uuid text,
    p_start_month date default date_trunc('month', current_date)::date,
...
declare v_ledger_id bigint; v_user_data text; v_end_month date;
begin
    v_user_data := utils.get_user();
    v_end_month := (p_start_month + (p_months_ahead || ' months')::interval)::date;
    select id into v_ledger_id from data.ledgers where uuid = p_ledger_uuid and user_data = v_user_data;
